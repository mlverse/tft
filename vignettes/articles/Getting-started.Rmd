...
sales_train <- sales %>% filter(Date <= (last_date - lubridate::weeks(16)))
sales_valid <- sales %>% filter(Date > (last_date - lubridate::weeks(16)),
...
```{r, eval = TRUE}
pred <- predict(fitted, new_data = sales_valid, past_data = sales_train)
