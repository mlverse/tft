...
batch_data <- function(recipe, df, total_time_steps = 12, device) {
  ...
  id <- recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::has_role("id")))
  time <- recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::has_role("time")))
  all_numeric <- c(recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::all_numeric()), empty_fun = function(x) {NULL}),
                   var_type_role[var_type_role$type == "date", "variable"] %>% unlist ) %>%
    as.character()
  all_nominal <- c(recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::all_nominal()), empty_fun = function(x) {NULL}),
                   var_type_role[var_type_role$type %in% c("logical", "other"), "variable"] %>% unlist) %>%
    as.character()
  known <- recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::has_role("known_input")), empty_fun = function(x) {NULL})
  observed <- recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::has_role("observed_input")), empty_fun = function(x) {NULL})
  static <- recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::has_role("static_input")), empty_fun = function(x) {NULL})
  ...
  target_numeric <- intersect(recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::all_outcomes())), all_numeric)
  target_categorical <- intersect(recipes::recipes_eval_select(var_type_role, quos=rlang::quos(recipes::all_outcomes())), all_nominal)
